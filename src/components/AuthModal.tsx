...
export const AuthModal: React.FC = () => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-900/70 backdrop-blur-xs overflow-y-auto">
      <div className="w-full max-w-sm bg-white rounded-3xl shadow-2xl overflow-hidden border border-slate-100 my-auto animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="p-4 sm:p-5 space-y-3.5">
          ...
          <button
            ...
            className="w-full py-2.5 px-4 bg-white hover:bg-slate-50 active:bg-slate-100 text-slate-800 text-xs font-semibold rounded-xl border border-slate-300 shadow-xs transition flex items-center justify-center gap-2.5 disabled:opacity-50 group"
          >
      ...
      {/* In-App Native Google Account Selector Sheet */}
      <GoogleAccountPickerModal
        isOpen={showGooglePicker}
        onClose={() => setShowGooglePicker(false)}
        savedAccounts={savedAccounts}
        onSelectAccount={async (email, displayName, photoUrl) => {
          await loginWithGoogle(email, displayName, photoUrl);
        }}
      />
    </div>
  );
